...
export async function generateMetadata({ params }) {
  try {
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/stores/${params.slug}`, {
      cache: 'no-store'
    });
    ...
    return getSEOTags({
      ...
      canonicalUrlRelative: `/tienda/${store.slug}`,
    });
  } catch (error) {
    return {
      title: "Error",
      description: "Error al cargar la tienda.",
    };
  }
}
...
async function getStore(slug) {
  try {
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/stores/${slug}`, {
      cache: 'no-store'
    });
...
export async function generateStaticParams() {
  // In a real app, you might want to fetch some popular stores
